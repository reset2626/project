...
def giordano_style_prediction_interval(
    ...
    for boot_idx in range(bootstrap_replications):
        innovation_sequence = sort_bootstrap_residuals(centered_high_residuals, horizon, rng)
        high_bootstrap_path = recursive_ann_forecast(
            ann_bundle["model"],
            ann_bundle["x_scaler"],
            ann_bundle["y_scaler"],
            estimated_high_train,
            horizon,
            ann_bundle["window_size"],
            clip_bounds=ann_bundle["clip_bounds"],
            innovation_sequence=innovation_sequence
        )
        bootstrap_paths[boot_idx, :] = low_forecast + high_bootstrap_path
...
def run_single_experiment(train_size=300, horizon=1, window_size=15, bootstrap_replications=300):
    ...
    interval_lower, interval_upper = giordano_style_prediction_interval(
        low_forecast,
        calibration_residuals["estimated_high"],
        calibration_residuals["ann_bundle"],
        calibration_residuals["centered_high_residuals"],
        horizon,
        alpha=0.05,
    ...
    calibration_residual_mean = np.mean(calibration_residuals["centered_high_residuals"])
    calibration_residual_std = np.std(calibration_residuals["centered_high_residuals"], ddof=1)
    ...
    return {
        ...
        "arima_order": arima_order,
        "calibration_residuals": calibration_residuals["centered_high_residuals"],
